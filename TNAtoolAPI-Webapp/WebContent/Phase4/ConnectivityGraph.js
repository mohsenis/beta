...
function openConGraph(){
	...
	var agencyCentroids= {};
	var results;
	var loaderHtml = '<img id="conGraphPreLoader" src="..images/287.GIF" alt="Page Loading" width="50" height="50" style="margin:auto" >';
...
//		var a1 = makeFeatureGroup(item.a1ID);
//		a1 = {lat:agencyCentroids[item.a1ID].lat, lng:agencyCentroids[item.a1ID].lng, hashCode:item.a1ID};
		...
		if (vertices[a1.hashCode] != undefined){
			...
			vertex1 = makeMarker(a1.lat, a1.lng);
			...
			vertex1.on('mouseover',function(e){
					vertex1.openPopup(); 
					toggleVertexNeighbors(vertexEdgesMap, vertex1, opc1, color1);
				})
				.on('mouseout',function(e){
					vertex1.closePopup(); 
					toggleVertexNeighbors(vertexEdgesMap, vertex1, opc0, color0);
				});
		...
		if (item.connections.size > 0){
			...
			if (vertices[a2.hashCode] != undefined){
				...
				vertex2 = makeMarker(a2.lat, a2.lng);
				...
				vertex2.on('mouseover',function(e){
					vertex2.openPopup(); 
						toggleVertexNeighbors(vertexEdgesMap, vertex2, opc1, color1);
					})
					.on('mouseout',function(e){
						vertex2.closePopup(); 
						toggleVertexNeighbors(vertexEdgesMap, vertex2, opc0, color0);
					});
			...
			var edge = L.polyline([vertex1.getLatLng(),edgeCentroid(vertex1,vertex2), vertex2.getLatLng()],{color: color0, opacity:opc0});
			edge.on('mouseover',function(e){
					edge.openPopup();
					toggleEdge(edgeVerticesMap, edge, opc1, color1);					
				})
				.on('mouseout',function(e){
					edge.closePopup();
					toggleEdge(edgeVerticesMap, edge, opc0, color0);
				});
	...
	$.ajax({
		...
		success: function(d){			
			$.each(d,function(index, item){
				agencies[item[0]] = item[1];
			});
		}
	});
	
	
	html += '<br><table style="font-size:100%"><tr><th>1</th><th>2</th><th>3</th><th>Agency</th></tr>' 
		
	$.each(agencies, function(id, name){
 ...
 */
function toggleEdge(edgeVerticesMap, e, o, c){
 ...
 * @param color (string)
 */
function toggleVertexNeighbors(agencyEdgesMap, v, o, c){
 ...
 * @param color (string)
 */
function toggleVertexConnections(agencyEdgesMap, edgeVerticesMap, v, o, c){
	v.setStyle({fillOpacity : o, fillColor:c});
	$.each(agencyEdgesMap[v.hashCode], function(index,item){
		if (item instanceof L.Polyline){
			if (item.options.opacity != o){
				...
				$.each(edgeVerticesMap[item.hashCode], function(index, vertex){
					if (vertex.options.fillOpacity != o){
						...
						toggleVertexConnections(agencyEdgesMap, edgeVerticesMap, vertex, o, c);
					}
				})
			}				
		}
...
function myFunction(input){
	...
	$.each($( ":input#" + input.id + ".conGraphCheckBox" ), function(index,item){
		if (item.dataset._case != myCase ){
			item.checked = false;
			toggleVertexConnections(vertexEdgesMap, edgeVerticesMap, vertices[item.id], opc0, color0);			
		}			
	});
	...
	if (myCase == 1){
		...
		if (isChecked){
			toggleVertexNeighbors(vertexEdgesMap, vertex, opc1, color1);
		}else{
			toggleVertexNeighbors(vertexEdgesMap, vertex, opc0, color0);
		}
	}else if (myCase == 3){
		if (isChecked){
			toggleVertexConnections(vertexEdgesMap, edgeVerticesMap, vertex, opc1, color1);
		}else{
			toggleVertexConnections(vertexEdgesMap, edgeVerticesMap, vertex, opc0, color0);
		}
	}
}
